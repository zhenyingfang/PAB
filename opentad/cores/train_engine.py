...
def train_video_one_epoch(
    ...
    model.train()
    for iter_idx, (data_dict, data_video_dict) in enumerate(zip(train_loader, train_video_loader)):
        optimizer.zero_grad()
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            losses = model(**data_dict, return_loss=True, data_video_dict=data_video_dict)
...
def train_point_one_epoch(
    ...
    model.train()
    for iter_idx, (data_dict, data_point_dict) in enumerate(zip(train_loader, train_point_loader)):
        optimizer.zero_grad()
        ...
        # forward pass
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            losses = model(**data_dict, return_loss=True, data_point_dict=data_point_dict)
...
def train_multi_one_epoch(
    ...
    model.train()
    for iter_idx, (data_dict, data_video_dict, data_point_dict) in enumerate(zip(train_loader, train_video_loader, train_point_loader)):
        optimizer.zero_grad()
        ...
        # forward pass
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_amp):
            losses = model(**data_dict, return_loss=True, data_video_dict=data_video_dict, data_point_dict=data_point_dict)
