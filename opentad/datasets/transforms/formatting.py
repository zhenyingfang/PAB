...
class ResizeFeat:
    ...
    def __call__(self, results):
        ...
        if "gt_points" in results.keys():
            resize_points = (results["gt_points"] / results["duration"]).clamp(min=0.0, max=1.0)
            resize_points *= tscale
            gt_labels = results["gt_labels"]
            point_labels = torch.zeros((tscale, 200)).long()
            for i, point in enumerate(resize_points):
                point_labels[int(point.item()), gt_labels[i]] = 1
