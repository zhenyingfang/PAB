...
class PointDETR(DeformableDETR):
    def __init__(
        self,
        projection,
        transformer,
    ...
    def post_processing(self, predictions, metas, post_cfg, ext_cls, **kwargs):
        ...
        ref_cls_labels = output['cls_labels'].squeeze(2)
        ref_cls_score = output['pred_decls']
        ref_cls_score = torch.gather(ref_cls_score, dim=1, index=ref_cls_labels.long())
        ref_cls_score = ref_cls_score.unsqueeze(2)

        pre_nms_topk = getattr(post_cfg, "pre_nms_topk", 200)
        bs, _, num_classes = pred_logits.shape

        # >> save decls results
        if output.get("pred_decls", None) is not None:
            pred_decls = output['pred_decls'].cpu().detach().numpy()

            for meta_idx in range(len(metas)):
                now_meta = metas[meta_idx]
                vid_name = now_meta['video_name']
                pred_vid = pred_decls[meta_idx]
                dst_path = f"exps/decls_res/{vid_name}.npy"
                if not isinstance(ext_cls, list):
                    pred_vid_max = np.max(pred_vid)
                    if pred_vid_max >= 0.5:
                        pred_vid = np.where(pred_vid >= 0.5, pred_vid, pred_vid * 0.01)
                    ext_cls.cuhk_data_score[vid_name] = pred_vid.tolist()
                # np.save(dst_path, pred_vid)

        # Select top-k confidence boxes for inference
        if self.with_act_reg:
            prob = pred_logits.sigmoid() * output["pred_actionness"] * ref_cls_score
            # prob = output["pred_actionness"]
        else:
        ...
        batch_labels = torch.gather(ref_cls_labels, 1, topk_boxes)
