...
class DeformableDETR(DETR):
    ...
    def forward_train(self, inputs, masks, metas, gt_segments, gt_labels, **kwargs):
        ...
        if kwargs.get('data_video_dict', None) is not None:
            x = torch.cat([x, kwargs['data_video_dict']['inputs']], dim=0)
            masks = torch.cat([masks, kwargs['data_video_dict']['masks']], dim=0)
        if kwargs.get('data_point_dict', None) is not None:
            x = torch.cat([x, kwargs['data_point_dict']['inputs']], dim=0)
            masks = torch.cat([masks, kwargs['data_point_dict']['masks']], dim=0)
    ...
    def forward_test(self, inputs, masks, metas=None, infer_cfg=None, gt_segments=None, gt_labels=None, **kwargs):
        if self.with_backbone:
            x = self.backbone(inputs)
        else:
            x = inputs
        ...
        output = self.transformer.forward_test(x, padding_masks, metas=metas, gt_segments=gt_segments, gt_labels=gt_labels, **kwargs)
