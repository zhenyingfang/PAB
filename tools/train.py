...
from opentad.cores import train_one_epoch, val_one_epoch, eval_one_epoch, build_optimizer, build_scheduler, train_multi_one_epoch, train_video_one_epoch, train_point_one_epoch
...
def main():
    ...
    # build omni dataset
    train_video_loader = None
    if cfg.dataset.get("train_video", None) is not None:
        train_video_dataset = build_dataset(cfg.dataset.train_video, default_args=dict(logger=logger))
        train_video_loader = build_dataloader(
            train_video_dataset,
            rank=args.rank,
            world_size=args.world_size,
            shuffle=True,
            drop_last=True,
            **cfg.solver.train_video,
        )

    train_point_loader = None
    if cfg.dataset.get("train_point", None) is not None:
        train_point_dataset = build_dataset(cfg.dataset.train_point, default_args=dict(logger=logger))
        train_point_loader = build_dataloader(
            train_point_dataset,
            rank=args.rank,
            world_size=args.world_size,
            shuffle=True,
            drop_last=True,
            **cfg.solver.train_point,
        )
    ...
    for epoch in range(resume_epoch + 1, max_epoch):
        ...
        # if epoch == 15:
        #     for param in model.module.transformer.decoupled_cls.parameters():
        #         param.requires_grad = False

        if train_video_loader is not None and train_point_loader is not None:
            train_video_loader.sampler.set_epoch(epoch)
            train_point_loader.sampler.set_epoch(epoch)
            train_multi_one_epoch(
                train_loader,
                model,
                optimizer,
                scheduler,
                ...
                scaler=scaler,
                train_video_loader=train_video_loader,
                train_point_loader=train_point_loader,
            )
        elif train_video_loader is not None:
            train_video_loader.sampler.set_epoch(epoch)
        ...
        elif train_point_loader is not None:
            train_point_loader.sampler.set_epoch(epoch)
            train_point_one_epoch(
                train_loader,
                model,
                optimizer,
                scheduler,
